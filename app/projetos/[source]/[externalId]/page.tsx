  ...
  return (
    <main id="conteudo" className="detailPage">
      ...
      <header className="projectHero">
        ...
        {project.friendlyTitle || project.shortDescription || project.practicalImpact ? <section className="aiSummary" aria-labelledby="ai-summary-title"><span className="aiLabel">Gerado por IA</span>{project.friendlyTitle ? <h1 id="ai-summary-title">{project.friendlyTitle}</h1> : null}{project.shortDescription ? <p>{project.shortDescription}</p> : null}{project.practicalImpact ? <section aria-labelledby="ai-practical-impact-title" className="aiSummary__impact"><h2 id="ai-practical-impact-title">O que muda na prática</h2><p>{project.practicalImpact}</p></section> : null}<small>Explicação gerada por IA a partir de registros oficiais. Confira a fonte original.</small></section> : null}
